...
def plot_response_curves(probed_input: torch.Tensor, probed_output: torch.Tensor, vector_levels: torch.Tensor, feature_ndx: int, plot_input=True):
...
def measure_nonhomogeneity(vector_levels, probed_output, feature_ndx, test_prop=0.9):
    ...
    return quad_reg_score - lin_reg_score, lin_reg_score, quad_reg_score
...
def get_nonhomog_grid(n_samples, n_feats, n_on, feature_vectors, nonlin_fn, feature_ndx):
    ...
    for i, scale in enumerate([2**x for x in range(10)]):
        for j, bias in enumerate([-(2**x) for x in range(8)]):
            ...
            nonhomogs[j, i], _, _ = measure_nonhomogeneity(vector_levels, probed_output, feature_ndx)
    ...
    for i, dims_per_feature in enumerate(dims_per_feature_l):
        for j, n_on in enumerate(n_on_l):
            ...
            for feature_ndx in range(n_feats_tested):
                ...
                probed_output = outputs @ feature_vectors.T
                nonhomog_score, _, _ = measure_nonhomogeneity(vector_levels, probed_output, feature_ndx)
...
def write_homogeneity_testing():
    ...
    output_fig = plot_response_curves(probed_input, probed_output, vector_levels, feature_ndx, plot_input=False)
    st.pyplot(output_fig) # For annotating on figma with lin and quadratic r^2 values

    nonhomog_score, lin_score, quad_score = measure_nonhomogeneity(vector_levels, probed_output, feature_ndx)
    st.write(f"## Nonhomogeneity Score: {nonhomog_score:.10}")
    st.write(f"## Linear R^2: {lin_score:.3}")
    st.write(f"## Quadratic R^2: {quad_score:.3}")
